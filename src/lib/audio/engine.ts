...
export class AudioEngine {
  ...
  async startDemo() {
    ...
    await this.resume();
    await this.playSpeaker(URL.createObjectURL(audioBufferToWav(buffer)));
  }
  ...
  async startFile(file: File) {
    ...
    await this.resume();
    await this.playSpeaker(URL.createObjectURL(file));
  }
  ...
  private setMonitor(kind: SourceKind) {
    if (this.silent) this.silent.gain.value = 0;
    if (this.speaker) {
      const live = kind === "demo" || kind === "file";
      this.speaker.muted = this.muted || !live;
      this.speaker.volume = this.muted || !live ? 0 : this.outputGain;
    }
  }
  ...
  private async playSpeaker(url: string) {
    this.stopSpeaker(false);
    this.objectUrl = url;
    const el = this.speakerEl();
    el.src = url;
    el.muted = this.muted;
    el.volume = this.muted ? 0 : this.outputGain;
    try { await el.play(); } catch { /* needs gesture */ }
  }

  private stopSpeaker(remove = true) {
    if (this.speaker) {
      this.speaker.pause();
      this.speaker.removeAttribute("src");
      this.speaker.load();
      if (remove) {
        this.speaker.remove();
        this.speaker = null;
